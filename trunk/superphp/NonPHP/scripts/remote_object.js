...
Moon.HttpClient = Moon.createClass();
Moon.HttpClient.prototype = {
    xmlhttp: null,
    userhandler: null,
    timeout_id: null,
    ...
    init: function() {
        try {
            ...
            if ( !success ) {
							throw new Error('Unable to create XMLHttpRequest.',1000);
            }
        }
    },
...
Moon.RPC.prototype = {
	initialize:function(service,method,params,callback){
		this.service = service;
		this.param = '';
		this.callback = Prototype.K;
		if (arguments.length == 4){
			this.param = this.setParam(method,params,callback);
		}
	},
	request:function(method,params,callback){
		var _this=this;
        var param = this.setParam(method,params);
		return new Ajax.Request(this.getUri(),{parameters: param, onComplete: function(response){callback(response.responseText.parseJSON());}});
	},
	setUri:function(service){
		this.service = service;
	},
	setParam:function(method,params){
		return 'method='.concat(method,'&','parameters','=',encodeURIComponent(params.toJSONString()));
	},
	getUri:function(){
		this.serviceUri = '/_JSON_'+('ajax'.concat('_',this.service,'.php').ucfirst());
		return this.serviceUri;
	}
};
var RPC = new Widgets.RPC('test');
