...
class SOSO_Util_File {
	...
	//文件大小超过了用户指定大小
	const UPLOAD_ERR_USER_SIZE = 10;

	public $name;

	public $info;
	
	//file handler if the file is opened
	public $handle,$lock,$path;

	static public $extensions = array('application/andrew-inset' => 'ez',
	 ...
	 * @param pMaxFileSize
	 */
	public static function allUpload($pPath = "", $pExtendName = array(), $pFileType = array(), $pMaxFileSize = 0){
		if (!count($_FILES)) return 0;
		$keys = array_keys($_FILES);
		$length = count($keys);
		$oFile = new self();
		$ret = 0;
		foreach($_FILES as $key=>$file){
			if (self::UPLOAD_ERR_OK == $file['error']){
				if ($pMaxFileSize > 0 && $file['size'] > $pMaxFileSize){
					$_FILES[$key]['error'] = self::UPLOAD_ERR_USER_SIZE;
					continue;
				}
				if (count($pFileType) > 0 && !in_array($_FILES[$key]['type'], $pFileType)) {
					$_FILES[$key]['error'] = self::UPLOAD_ERR_EXTENSION;
					continue;
				}
				$pos = strrpos($_FILES[$key]['name'], ".");
				if ($pos > 0) {
					$_FILES[$key]['extend'] = strtolower(substr($_FILES[$key]['name'],$pos+1));
					$_FILES[$key]['caption'] = substr($_FILES[$key]['name'],0,strlen($_FILES[$key]['name'])-strlen($_FILES[$key]['extend'])-1);
				}
				else {
					$_FILES[$key]['extend'] = '';
					$_FILES[$key]['caption'] = $_FILES[$key]['name'];
				}
				if (count($pExtendName) > 0 && !in_array($_FILES[$key]['extend'], $pExtendName)) {
					$_FILES[$key]['error'] = self::UPLOAD_ERR_EXTEND;
					continue;
				}
				$_FILES[$key]['new_file'] = md5_file($_FILES[$key]['tmp_name']);
				if(rename($_FILES[$key]['tmp_name'], "{$pPath}{$_FILES[$key]['new_file']}")) {
					chmod ("{$pPath}{$_FILES[$key]['new_file']}", 0644);
				}
				$return ++;
			}elseif(self::UPLOAD_ERR_NO_FILE == $file['error']){
				unset($_FILES[$key]);
			}
		}
	}
	 ...
	 * @param pContent
	 */
	public static function writeFile($pFilename, $pContent,$flag=null){
		if($flag) return file_put_contents($pFilename, $pContent,$flag);
		return file_put_contents($pFilename,$pContent);
	}
	 ...
	 * @param pDir
	 */
	public static function createDir($pDir,$mode=0644,$recursive=true){
		if(!file_exists($pDir)) return mkdir($pDir,$mode,$recursive);
		return false;
	}

	public function get_content_type($pFilename) {
		$extension = substr(strtolower($this->getExtension($pFilename)),1);
		if(false !== ($key = array_search($extension, self::$extensions))) {
			return $key;
		}
		else {
